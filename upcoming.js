...
const upcomingEvents = eventsCards.filter(events => events.date >= data.currentDate)
...
const cardsForCategory = upcomingEvents.filter( fn )
...
$checkboxs.addEventListener( 'change', (event) => {
    ...
    const cardsFilter = filterCards( upcomingEvents, checked )
